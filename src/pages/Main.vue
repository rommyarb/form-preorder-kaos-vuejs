	...
	export default {
		...
		watch: {
			"alamat.provinsi": function (value) {
				...
				Axios.get("https://x.rajaapi.com/MeP7c5ne" + this.token + "/m/wilayah/kabupaten?idpropinsi=" + id)
					.then(r => {
			...
			'alamat.kota': function (value) {
				...
				Axios.get("https://x.rajaapi.com/MeP7c5ne" + this.token + "/m/wilayah/kecamatan?idkabupaten=" + id)
					.then(
						r => {
			...
			submit() {
				...
				db.collection('pre_order_kaos').add(this.form).then(() => {
					...
					this.$router.push({
						name: 'success',
						params: {
							jumlah_item: this.form.kaos_.length
						}
					})
				}).catch(e => {
